  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.55)', zIndex: 300, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 20 }} onClick={e => e.target === e.currentTarget && close()}>
      <div style={{ background: 'var(--black)', color: 'white', borderRadius: 16, padding: '32px 28px', maxWidth: 380, width: '100%', textAlign: 'center', position: 'relative', boxShadow: '0 20px 60px rgba(0,0,0,.4)' }}>
        ...
        <div style={{ fontSize: 15, lineHeight: 1.6, marginBottom: 6 }}>Get <b>$10 OFF</b> Delivery Fees on Your First Order</div>
        <div style={{ display: 'inline-block', background: 'var(--gold)', color: 'var(--black)', fontWeight: 800, letterSpacing: 1, padding: '6px 16px', borderRadius: 8, margin: '8px 0 18px', fontSize: 15 }}>WELCOME10</div>
        <div style={{ fontSize: 15, lineHeight: 1.6, marginBottom: 4 }}>Plus, receive <b style={{ color: 'var(--gold)' }}>1 FREE COOLER CAN</b> with your order</div>
